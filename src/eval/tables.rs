//! PeSTO piece-square tables — tapered MG/EG.
//!
//! Source: Ronald Friederich (RofChade) "PeSTO's Evaluation Function" as
//! published on chessprogramming.org "PeSTO's Evaluation Function" (via
//! Pawel Koziol's TSCP port). The C source uses `a8 = 0` indexing; the
//! tables below are flipped via `sq ^ 56` so that in Bitwark `a1 = 0`
//! (Square::index) is used directly for White and `sq ^ 56` for Black.
//!
//! Material values are the PeSTO `mg_value[] / eg_value[]`:
//!   MG: pawn 82, knight 337, bishop 365, rook 477, queen 1025, king 0
//!   EG: pawn 94, knight 281, bishop 297, rook 512, queen 936, king 0
//!
//! PSTs are pure piece-square deltas (material not folded in) so that the
//! `eval` breakdown can show Material vs PieceSq separately.

/// MG material values (centipawns) indexed by PieceType (Pawn=0 .. King=5).
pub const MG_VALUE: [i32; 6] = [82, 337, 365, 477, 1025, 0];
/// EG material values.
pub const EG_VALUE: [i32; 6] = [94, 281, 297, 512, 936, 0];

/// Phase weight per PieceType (Pawn 0, Knight 1, Bishop 1, Rook 2, Queen 4, King 0).
/// Total phase = min(24, Σ weights over all pieces both colours).
pub const PHASE_WEIGHT: [i32; 6] = [0, 1, 1, 2, 4, 0];

// ---------------------------------------------------------------------------
// Piece-square tables — White perspective, a1 = 0.
// Each table is indexed [sq] where sq = Square::index (0 = a1).
// Black mirrors via sq ^ 56.
// ---------------------------------------------------------------------------
